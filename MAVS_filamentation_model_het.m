...
axis([0 tspan 1e-3 1e3])
xlabel('Time')
legend({'Glu^9^3Ala^2^7^1' 'Glu^9^3/Gln^9^3Gln^2^7^1 het' 'Gln^9^3Gln^2^7^1'}, ...
...
if strcmpi(model,'Glu93Gln271')
    dMAVS_dt = k_a - k_p*(fb/(fb+MAVSclv1))*MAVS.^n - 1.5*k_c*C3pro*MAVS;
    dpolyMAVS_dt = k_p*(fb/(fb+MAVSclv1))*MAVS.^n - k_d/n*polyMAVS;
    dISGs_dt = k_t/n*polyMAVS - k_d*ISGs;
    dC3pro_dt = k_a;
    dMAVSclv1_dt = k_c*C3pro*MAVS;
    dMAVSclv2_dt = 0.5*k_c*C3pro*MAVS;   %polymorphism is heterozygous
end
